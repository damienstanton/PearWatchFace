...
class WatchFaceService : CanvasWatchFaceService() {
    ...
    private inner class Engine : CanvasWatchFaceService.Engine() {
        ...
        private var mSimpleTicksModule: SimpleTicksModule? = null
        ...
        fun onCreate(holder: SurfaceHolder) {
            ...
            mSimpleTicksModule = SimpleTicksModule(mStyle)
            ...
            val color = mPrefs!!.getInt("settings_simple_color_value", Color.parseColor("#00BCD4"))
            val accentColor = mPrefs!!.getInt("settings_simple_accent_color_value",
        ...
        private fun setBounds() {
            var inset = 10
            if (SETTINGS_MODE == 3) {
                inset += (mWidth * 0.07f).toInt()
            }

            val screenBounds = Rect(inset - 5, inset - 5,
                    mWidth - inset + 5, mHeight - inset + 5)
            if (SETTINGS_MODE < 3) {
                inset += if (mStyle == 3) (mWidth * 0.07f).toInt() else 0
            }
